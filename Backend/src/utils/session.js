...
export async function createSession(user, req, res) {
  ...
  await Session.create({ user: user._id, tokenHash: hashToken(token), expiresAt: new Date(Date.now() + maxAge) });
...
export function publicUser(user) {
  return { id: String(user._id), name: user.name, shopName: user.shopName, email: user.email };
}
