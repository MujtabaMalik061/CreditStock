...
router.get('/', asyncRoute(async (req, res) => {
  ...
  const days = Array.from({ length: 7 }, (_, index) => {
    ...
    return { date: day.toISOString().slice(0, 10), label: day.toLocaleDateString('en-PK', { weekday: 'short' }), sales: 0, count: 0 };
  });
  let todaySales = 0, todaySaleCount = 0, weeklySales = 0, estimatedProfit = 0;
  for (const sale of sales) {
    const saleDay = new Date(sale.createdAt); saleDay.setHours(0, 0, 0, 0);
    const index = Math.round((saleDay - weekStart) / 86400000);
    if (days[index]) { days[index].sales += sale.total; days[index].count += 1; }
    if (saleDay.getTime() === start.getTime()) { todaySales += sale.total; todaySaleCount += 1; }
    weeklySales += sale.total;
    estimatedProfit += sale.total - (sale.unitCost ?? sale.product?.costPrice ?? 0) * sale.quantity;
  }
  res.json({
    todaySales, todaySaleCount, weeklySales, estimatedProfit,
    outstandingCredit: Object.values(due).reduce((a, b) => a + b, 0),
    productCount: products.length, customerCount,
