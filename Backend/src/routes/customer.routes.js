...
router.get(
  ...
  asyncRoute(async (req, res) => {
    const [items, due] = await Promise.all([
      Customer.find({ owner: req.owner }).sort({ name: 1 }).lean(),
      balances(req.owner),
    ]);
    res.json(
      items.map((item) => ({ ...item, balance: due[String(item._id)] || 0 })),
    );
  }),
);
router.post(
...
router.get(
  "/:id/ledger",
  asyncRoute(async (req, res) => {
    if (!(await Customer.exists({ _id: req.params.id, owner: req.owner })))
      return fail(res, "Customer not found", 404);
    res.json(
      await Transaction.find({ owner: req.owner, customer: req.params.id })
